...
CREATE VIEW games AS SELECT player_ID, name, tournament_ID, COUNT(match_ID) AS games FROM \
players LEFT JOIN matches ON (player_ID = matches.winner_ID OR \
player_ID = matches.loser_ID) GROUP BY player_ID, tournament_ID;

CREATE VIEW wins AS SELECT player_ID, COUNT(winner_ID) AS wins FROM \
players LEFT JOIN matches ON player_ID = winner_ID GROUP BY players.player_ID;
